...
function render() {
  ...
  els.selectedMonth.textContent = formatMonthYear(selected);
...
function renderHabitList(container, items, options) {
  ...
  const statusWeight = { done: 0, missed: 1, none: 2 };
  const orderedItems = options.todayOnly
    ? [...items].sort((left, right) => statusWeight[getRecordStatus(left, key)] - statusWeight[getRecordStatus(right, key)])
    : items;

  orderedItems.forEach((habit) => {
...
function initializeGoogleCloud() {
  ...
  firebase.onUserChanged((user) => {
    ...
    if (user) {
      ...
      firebase.authReady.then((settledUser) => {
        if (!settledUser) syncScreenTimeBonus();
      });
    }
  });
}
